...
export async function runMigration(config: MigrateConfig): Promise<MigrateStats> {
	...
	try {
		...
		// MUST run AFTER createIndexes(): the second UPDATE uses a
		// correlated EXISTS subquery and without idx_posts_thread SQLite
		// would full-scan posts per thread row (≈9.5M × millions = hung
		// import).
