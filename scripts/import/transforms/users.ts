 ...
 */
function loadAndIndex<T extends { uid: number }>(
  file: string,
  table: string
): Map<number, T> {
...
export async function transformUsers(
  ...
  const memberMap = loadAndIndex<CommonMember>(
    "main_small.sql.gz",
    "pre_common_member"
  );
  ...
  const countMap = loadAndIndex<MemberCount>(
    "user_extra.sql.gz",
  ...
  const profileMap = loadAndIndex<MemberProfile>(
    "user_extra.sql.gz",
  ...
  const statusMap = loadAndIndex<MemberStatus>(
    "user_extra.sql.gz",
