...
export function parseMySQLDump(
	...
	const createTableRegex = new RegExp(`CREATE TABLE \`${tableName}\`\\s*\\(([^;]+)\\)`, "i");
 ...
 */
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Parser state machine needs all branches
...
if (import.meta.main) {
	...
	if (args.length < 2) {
		console.log(
			"Usage: bun run parse-dump.ts <dump.sql.gz> <table_name> [--limit N] [--offset N] [--json]",
		);
		process.exit(1);
	}
