...
> = {
	...
	forums: {
		...
		mysqlTables: ["pre_forum_forum", "pre_forum_forumfield", "pre_forum_moderator"],
...
function transformUsers(_rows: unknown[]): unknown[] {
...
function transformForums(_rows: unknown[]): unknown[] {
...
// biome-ignore lint/correctness/noUnusedVariables: Will be used when importing data
...
async function verifyTable(tableName: string): Promise<{
	...
	const valid = expected ? count >= expected.min && count <= expected.max : count > 0;
...
async function main() {
	...
	try {
		const markerResult = await executeD1("SELECT value FROM _test_marker WHERE key = 'env'");
		const parsed = JSON.parse(markerResult);
		const env = parsed[0]?.results?.[0]?.value;
		if (env !== "test") {
			console.error("❌ SAFETY CHECK FAILED: _test_marker.env != 'test'. Aborting!");
			process.exit(1);
		}
