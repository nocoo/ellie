 ...
 *   - tests/integration/worker/**    : L2 test calls (numerator)
...
const L2_TEST_DIR = resolve(REPO_ROOT, "tests/integration/worker");
...
function printSummary(rep: CoverageReport): void {
	...
	console.log("Test glob         : tests/integration/worker/*.test.ts");
...
function renderMarkdown(rep: CoverageReport): string {
	...
	lines.push("> to `apps/worker/src/index.ts` or `tests/integration/worker/*.test.ts`.");
	lines.push("");
