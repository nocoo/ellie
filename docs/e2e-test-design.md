...
**解决方案**: Storage State Injection (CI/Setup Project 生成)

```typescript
// playwright.config.ts — Setup project generates auth state
export default defineConfig({
  projects: [
    {
      name: "setup",
    ...
    },
    {
      ...
      dependencies: ["setup"],
      use: {
        storageState: ".auth/admin.json",  // Generated by setup, NOT committed
      },
    },
  ],
});
...
async function globalSetup() {
  // Option A: E2E bypass mode (recommended for CI)
  if (process.env.E2E_ADMIN_BYPASS === "true") {
    // No auth needed — proxy allows all admin routes
    return;
  }
  
  // Option B: Manual OAuth flow (local dev)
  // Run once: npx playwright test --project=setup --headed
  // Then manually complete Google OAuth, state saved to .auth/admin.json
