...
- `proxy.ts` 检查 OAuth session + admin whitelist
- `(admin)/layout.tsx` 服务端调用 `auth()` + `resolveAdmin()`，非 admin 重定向

**解决方案 A: Storage State (推荐)**

使用 Playwright setup fixture 生成认证状态：
...
```typescript
// 1. apps/web/src/proxy.ts — 绕过 proxy 层
export function resolveProxyAction(...) {
  // E2E bypass for admin routes
  if (process.env.E2E_ADMIN_BYPASS === "true" && isAdminRoute(pathname)) {
    return "next";
  }
  // ... existing logic
}

// 2. apps/web/src/app/(admin)/layout.tsx — 绕过 layout 层
export default async function AdminLayout({ children }: { children: ReactNode }) {
  // E2E bypass
  if (process.env.E2E_ADMIN_BYPASS === "true") {
...
**Important**: `.auth/` 目录必须加入 `.gitignore`，storage state 包含 session cookies，不得提交到仓库。
