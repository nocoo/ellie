...
缓存监控不再持续采集 hit/miss/load、物理 KV 操作或应用 D1 观测，也不再写入/清理小时与分钟监控表。平台 D1 成本趋势以 Cloudflare 原生监控为准。

- Admin 默认只读 `admin:kv:snapshot:v1`，显示上次采集时间；无快照时提示管理员生成，不自动扫描或填充。
- 管理员点击“生成快照”，经带管理员认证及 CSRF 防护的 `POST /api/admin/kv/snapshot`，执行有界 KV metadata 扫描，保存一个 JSON 并覆盖上一份，不写 D1。每个前缀沿用扫描上限，保留脱敏和未知/部分覆盖标记。
- 快照没有自动过期，不代表读取时的实时状态。首次采集、失败重试都必须显式触发；失败不先删除旧快照，KV 写入未确认时报错。跨地区仍遵循 KV 最终一致性。
- 连续行为趋势已停止；“历史观测”只展示旧数据，不把状态快照当成命中率，不补造历史。旧表原样保留，本次不做数据清理迁移。
- 单条预览、删除、重建和成组失效继续按需使用；写操作仍保留原有权限与审计。正常内容浏览不会为缓存监控产生 D1 写入。

以上取代前文持续采集与七天自动清理的运行约定；相关段落保留为此前实现记录。
