...
- Server Action 使用 `getToken()` 解密 cookie 获取 workerJwt（见 §3.2）
...
> Server Action 通过 `getToken()` 解密 cookie 获取 `workerJwt`（见 §3.2）。
...
session({ session, token }) {
  if (token.provider === "credentials") {
    ...
    // 客户端 useSession() 看不到它
    // 服务端通过 getToken(cookie) 解密读取（见 §3.2）
  } else {
    // Google OAuth — 保持现有行为
    session.user.id = token.sub ?? "";
...
```
Browser → Server Action → getToken(cookie).workerJwt → 调用 Worker API → 返回结果
```
...
**使用 `getToken()` 解密 NextAuth cookie**，不通过 `auth()` 返回值。
...
#### 实现：使用 getToken() 解密 cookie

使用 `@auth/core/jwt` 的 `getToken()` 解密 NextAuth session cookie，获取完整的 JWT payload（含 `workerJwt`）。这是纯密码学操作，不触发任何 callback，不修改 cookie。

**为什么用 `getToken()` 而非直接 `decode()`？** Auth.js 在 session cookie 超过浏览器 4KB 限制时，会将 JWE 拆分为 `authjs.session-token.0`, `.1`, `.2` ... 多个分片 cookie。`getToken()` 内部使用 `SessionStore` 自动按后缀排序拼装分片后再调用 `decode()`，而直接用 `cookies().get(cookieName)` 只能读到基础名，分片时会拿到 `null` 或残缺内容。
...
/**
 * 使用 getToken() 从 NextAuth session cookie 中解密完整 JWT payload。
 *
 * getToken() 内部：
 * 1. 从 request headers 中提取 cookie
 * 2. 使用 SessionStore 自动拼装分片 cookie（.0, .1, ...）
 * 3. 调用 decode() 解密 JWE → 返回完整 JWT payload
 *
 * 不触发 jwt/session callback，不写 cookie。
 */
...
| 关注点 | 说明 |
|--------|------|
| workerJwt 不到达浏览器 | cookie 是 httpOnly JWE（加密），浏览器 JS 无法解密；`getToken()` 仅在服务端执行 |
| refresh 链路完整 | proxy 层在每次页面请求时已通过 jwt callback 刷新并回写 cookie |
| 分片 cookie 兼容 | `getToken()` 内部 `SessionStore` 自动拼装 `.0`, `.1` ... 分片 |
| 无竞态风险 | `getToken()` 是只读操作，不写 cookie，不触发副作用 |
| cookie name / salt 匹配 | `secureCookie` 参数让 `getToken()` 内部选择正确的 cookie name 前缀 |

#### ⚠️ 边界场景：长时间停留后直接提交

Proxy 层的 refresh 仅在**页面导航**时触发。如果用户打开编辑页后长时间未刷新（超过 Worker JWT 7 天有效期），直接点击提交，Server Action 拿到的 `workerJwt` 已过期，Worker 会返回 `TOKEN_EXPIRED`（401）。

**兜底策略：Server Action 层 TOKEN_EXPIRED 重试**
...
/**
 * 调用需认证的 Worker API，内置 TOKEN_EXPIRED 兜底。
 *
 * 1. 用 getWorkerJwt() 获取 cookie 中的 workerJwt
 * 2. 调用 Worker API
 * 3. 如果 Worker 返回 TOKEN_EXPIRED：
 *    a. 用 cookie 中的 workerRefreshToken 调用 Worker refresh 端点
 *    b. 用新 token 重试一次原请求
 *    c. 注意：新 token 无法写回 NextAuth cookie（Server Action 无法写 cookie）
 *       下次页面导航时 proxy 层会再次 refresh（使用 NextAuth cookie 中的旧 refreshToken
 *       可能已失效 → token.error = "RefreshTokenExpired" → SessionGuard 登出）
 * 4. 如果 refresh 也失败 → 返回 NOT_AUTHENTICATED，前端引导重登
 */
...
import { authFetch } from "@/lib/forum-auth";

/** 发新帖 — 需要 Worker JWT，内置 TOKEN_EXPIRED 兜底 */
export async function createThread(forumId: number, subject: string, content: string) {
  // authFetch() 内部：
  // 1. getToken() 解密 cookie 获取 workerJwt（含分片拼装）
  // 2. 调用 Worker API
  // 3. 如果 TOKEN_EXPIRED → 自动 refresh + 重试
  return authFetch<{ threadId: number }>("/api/v1/threads", { forumId, subject, content });
}
```
...
Server Action 中获取 workerJwt:
  → getToken() 解密 NextAuth cookie（含分片拼装，不触发 callback）
...
| `apps/web/src/lib/forum-auth.ts` | Server-only：getWorkerJwt() + getCurrentForumUser() + authFetch()（基于 getToken()） |
...
| Worker JWT 不暴露给浏览器 | 存在 NextAuth JWT（httpOnly JWE cookie），session callback 不输出，服务端通过 `getToken(cookie)` 获取 |
