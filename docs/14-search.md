...
[search.ts](../apps/worker/src/handlers/search.ts)负责规范化关键词、limit 和游标，并获取当前用户的可见性范围。搜索成员集合由[统一目录读取模块](../apps/worker/src/lib/cache/catalog-read.ts)的 `search:threads` family 缓存 60 秒，key 包含关键词、分页、limit 和可见性范围。超过接纳长度的合法查询走受限回源，不填充 KV。

缓存只保存主题 ID、排序游标和分页信息。响应前批量检查当前主题与版块可见性，组合主题实体、动态统计和用户 mini；不会缓存或复用越权的完整搜索响应。用户资料读取统一接入，不再依赖 `USE_KV_USER_CACHE` 分支，也不在 KV 故障时有意省略头像。

FTS 查询失败返回错误，不写入伪造空结果；空搜索结果使用相同的 60 秒档位。所有合法游标页均可复用，并通过[目录读取测试](../apps/worker/tests/unit/lib/cache/catalog-read.test.ts)验证过期、作用域、当前权限和只装载缺失实体。

完整生命周期、失效和回退规则见[统一缓存架构](20-worker-kv-reference.md)。源码是 SQL 与 DTO 的唯一实现，不在本文复制整份 handler。
