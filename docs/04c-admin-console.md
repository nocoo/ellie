...
├── users/route.ts                      # GET (list) / POST (batch ops)
...
```
6.1 API 代理基础 ──→ 6.2 共享组件 ──→ 6.3 Dashboard
                                    ──→ 6.4 Users
                                    ──→ 6.5 Threads
                                    ──→ 6.6 Posts
                                    ──→ 6.7 Forums
...
| # | Commit | 内容 | 测试 |
|---|--------|------|------|
| 6.2.1 | `feat: add admin data table component` | `components/admin/admin-data-table.tsx` — 通用表格 + 行选择 | ✅ L1: 列渲染 + 选择状态 |
| 6.2.2 | `feat: add admin pagination + filters + batch bar` | `admin-pagination.tsx` + `admin-filters.tsx` + `admin-batch-bar.tsx` + `admin-confirm-dialog.tsx` | ✅ L1: 分页计算 + URL 驱动 |
| 6.2.3 | `refactor: update admin sidebar with grouped nav` | 重建 `admin-sidebar.tsx` — 分组导航（内容管理 / 安全管理），9 个导航项 | ✅ L1: 导航项 + active 状态 |
| 6.2.4 | `refactor: extract stat-card component` | 从 `admin/page.tsx` 提取 `stat-card.tsx` 为独立组件 | ✅ L1: 渲染 |

---

### 6.3 Dashboard

替换 Mock 数据源为 Stats 端点。

| # | Commit | 内容 | 测试 |
|---|--------|------|------|
| 6.3.1 | `refactor: rewrite dashboard with stats endpoint` | 重建 `viewmodels/admin/dashboard.ts`（调用 adminApiClient）+ 重建 `admin/page.tsx`（StatCards + Detail Cards + Quick Links） | ✅ L1: fetchDashboardStats |

> 删除旧 `fetchDashboardData`（依赖 Mock repos）、`aggregateTrend`、`TrendPoint`。
...
| # | Commit | 内容 | 测试 |
|---|--------|------|------|
| 6.4.1 | `feat: add user admin api routes` | `app/api/admin/users/route.ts` + `[id]/route.ts` + `[id]/ban/route.ts` + `[id]/nuke/route.ts` + `batch-status/route.ts` + `batch-role/route.ts` | ✅ L1: 代理转发 |
| 6.4.2 | `refactor: rewrite user management viewmodel` | 重建 `viewmodels/admin/users.ts` — 全部 7 个函数调用 adminApiClient | ✅ L1: 全函数 |
| 6.4.3 | `refactor: rewrite user management page + components` | 重建 `admin/users/page.tsx` + 新建 `user-edit-dialog.tsx` + 重建 filters/actions 组件，使用 AdminDataTable + AdminPagination + AdminBatchBar | ✅ L1: 组件渲染 |

> 删除旧 `admin-user-actions.tsx`、`admin-user-filters.tsx`、`user-management.ts`。
...
| # | Commit | 内容 | 测试 |
|---|--------|------|------|
| 6.5.1 | `feat: add thread admin api routes` | `app/api/admin/threads/route.ts` + `[id]/route.ts` + `batch-delete/route.ts` + `batch-move/route.ts` | ✅ L1: 代理转发 |
| 6.5.2 | `feat: add thread management viewmodel + page` | 新建 `viewmodels/admin/threads.ts` + `admin/threads/page.tsx` + `thread-edit-dialog.tsx`，8 个筛选器，批量删除/移动 | ✅ L1: VM 函数 + 组件渲染 |
...
| # | Commit | 内容 | 测试 |
|---|--------|------|------|
| 6.6.1 | `feat: add post admin api routes` | `app/api/admin/posts/route.ts` + `[id]/route.ts` + `batch-delete/route.ts` | ✅ L1: 代理转发 |
| 6.6.2 | `feat: add post management viewmodel + page` | 新建 `viewmodels/admin/posts.ts` + `admin/posts/page.tsx` + `post-edit-dialog.tsx`，4 个筛选器，批量删除（含 skipped 提示） | ✅ L1: VM 函数 + 组件渲染 |

> 删除旧 `admin/content/page.tsx`、`content-moderation.ts`、`admin-content-actions.tsx`、`admin-content-filters.tsx`。
...
| # | Commit | 内容 | 测试 |
|---|--------|------|------|
| 6.7.1 | `feat: add forum admin api routes` | `app/api/admin/forums/route.ts` + `[id]/route.ts` + `[id]/merge/route.ts` + `reorder/route.ts` | ✅ L1: 代理转发 |
| 6.7.2 | `refactor: rewrite forum management viewmodel` | 重建 `viewmodels/admin/forums.ts` — 全部 7 个函数调用 adminApiClient | ✅ L1: 全函数 |
| 6.7.3 | `refactor: rewrite forum management page + components` | 重建 `admin/forums/page.tsx`（树形视图）+ 新建 `forum-create-dialog.tsx` + `forum-merge-dialog.tsx` + 重建 actions 组件，支持 create / edit / delete / merge / reorder | ✅ L1: 组件渲染 |

> 删除旧 `admin-forum-actions.tsx`、`forum-management.ts`。
...
| # | Commit | 内容 | 测试 |
|---|--------|------|------|
| 6.8.1 | `feat: add attachment admin api routes` | `app/api/admin/attachments/route.ts` + `[id]/route.ts` + `batch-delete/route.ts` | ✅ L1: 代理转发 |
| 6.8.2 | `feat: add attachment management viewmodel + page` | 新建 `viewmodels/admin/attachments.ts` + `admin/attachments/page.tsx`，4 个筛选器，批量删除 | ✅ L1: VM 函数 + 组件渲染 |
...
| # | Commit | 内容 | 测试 |
|---|--------|------|------|
| 6.9.1 | `feat: add ip-ban admin api routes` | `app/api/admin/ip-bans/route.ts` + `[id]/route.ts` + `batch-delete/route.ts` + `check-ip/route.ts` | ✅ L1: 代理转发 |
| 6.9.2 | `feat: add ip-ban management viewmodel + page` | 新建 `viewmodels/admin/ip-bans.ts` + `admin/ip-bans/page.tsx` + `ip-ban-create-dialog.tsx` + IP 检测工具 UI | ✅ L1: VM 函数 + 组件渲染 |
...
| # | Commit | 内容 | 测试 |
|---|--------|------|------|
| 6.10.1 | `feat: add censor-word admin api routes` | `app/api/admin/censor-words/route.ts` + `[id]/route.ts` + `batch-delete/route.ts` + `test/route.ts` | ✅ L1: 代理转发 |
| 6.10.2 | `feat: add censor-word management viewmodel + page` | 新建 `viewmodels/admin/censor-words.ts` + `admin/censor-words/page.tsx` + `censor-word-create-dialog.tsx` + 内容测试工具 UI | ✅ L1: VM 函数 + 组件渲染 |
...
| 6.6 | Posts | 2 | 15 |
| 6.7 | Forums | 3 | 18 |
| 6.8 | Attachments | 2 | 20 |
| 6.9 | IP Bans | 2 | 22 |
| 6.10 | Censor Words | 2 | 24 |
| 6.11 | 收尾 | 3 | **27** |
...
| 文件 | 变更 |
|------|------|
| `components/layout/admin-sidebar.tsx` | 分组导航（内容管理/安全管理），9 项 |
| `viewmodels/admin/dashboard.ts` | 改为调用 Stats 端点 |
| `viewmodels/admin/users.ts`（原 user-management.ts） | 改为调用 Worker API |
| `viewmodels/admin/forums.ts`（原 forum-management.ts） | 改为调用 Worker API |
| `app/(admin)/admin/page.tsx` | StatCards + Detail Cards + Quick Links |
| `app/(admin)/admin/users/page.tsx` | 使用 AdminDataTable + 完整操作 |
| `app/(admin)/admin/forums/page.tsx` | 支持 create/delete/merge/reorder |
| `app/(admin)/layout.tsx` | 强化权限守卫 |
...
|------|------|
| `lib/admin-api.ts` | 服务端 Worker API 客户端 |
| `lib/admin-proxy.ts` | API Route 代理工厂函数 |
...
| `viewmodels/admin/threads.ts` | 帖子 ViewModel |
| `viewmodels/admin/posts.ts` | 回帖 ViewModel |
| `viewmodels/admin/attachments.ts` | 附件 ViewModel |
| `viewmodels/admin/ip-bans.ts` | IP 封禁 ViewModel |
| `viewmodels/admin/censor-words.ts` | 敏感词 ViewModel |
...
| `components/admin/stat-card.tsx` | 统计卡片（从 page 提取） |
...
| `viewmodels/admin/content-moderation.ts` | 被 threads.ts + posts.ts 替代 |
| `components/admin/admin-content-actions.tsx` | 被各实体 actions 替代 |
| `components/admin/admin-content-filters.tsx` | 被通用 admin-filters.tsx 替代 |
| `components/admin/admin-user-actions.tsx` | 被 user-edit-dialog + 重建的 actions 替代 |
| `components/admin/admin-user-filters.tsx` | 被通用 admin-filters.tsx 替代 |
| `components/admin/admin-forum-actions.tsx` | 被 forum-create-dialog + 重建的 actions 替代 |
