...
- **VPS**: Legacy Discuz server (set `MIGRATION_SSH_HOST`)
- **SSH**: `ssh $MIGRATION_SSH_USER@$MIGRATION_SSH_HOST` (default key)
- **MySQL 数据库**:
  - `$MIGRATION_MYSQL_DB` — Discuz 主库 (default: `db_main`)
  - `$MIGRATION_MYSQL_UC_DB` — UCenter 用户库 (default: `db_ucenter`)
...
```bash
ssh $MIGRATION_SSH_USER@$MIGRATION_SSH_HOST
```

### 导出版主数据

```bash
mysqldump -u root $MIGRATION_MYSQL_DB pre_forum_moderator \
...
```bash
# 导出所有 PM 相关表
mysqldump -u root $MIGRATION_MYSQL_UC_DB \
  uc_pm_indexes \
  uc_pm_lists \
  uc_pm_members \
  uc_pm_messages_0 \
...
```bash
# 从本地执行
scp $MIGRATION_SSH_USER@$MIGRATION_SSH_HOST:/tmp/moderator.sql.gz reference/db/
scp $MIGRATION_SSH_USER@$MIGRATION_SSH_HOST:/tmp/pm.sql.gz reference/db/
```
...
npx wrangler d1 execute YOUR_TEST_D1_DATABASE \
...
```bash
npx wrangler d1 migrations apply YOUR_TEST_D1_DATABASE \
