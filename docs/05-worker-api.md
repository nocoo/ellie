...
>
> **Admin 认证架构**：浏览器仅持有 HttpOnly + Secure 的 Google OAuth session cookie。Next.js 服务端验证 Google session + `ADMIN_EMAILS` 白名单后，直接以 Key B 调用 Worker。Worker 不感知 Admin 身份——信任 Key B 即可。
...
### 认证 API（论坛用户，Key A 体系）

#### Auth（论坛用户认证）

| 方法 | 路径 | 说明 | 请求体 |
|------|------|------|--------|
| POST | /api/v1/auth/login | 论坛用户登录 | `{ username, password }` |
...
### 版主操作 API（Key A + 论坛 JWT，role ∈ {1, 2, 3}）

**重要**：`/api/v1/moderation/*` 走 **Key A** 体系，使用论坛用户 JWT 认证。版主在论坛前端（Web）中执行这些操作，不需要进入 Admin Console。
- Admin (1) / SuperMod (2) / Mod (3)：均可在论坛前端执行版主操作
- Mod (3) **不能**进入 Admin Console（Admin Console 使用 Key B + Google OAuth，与论坛用户无关）
...
**重要**：`/api/admin/*` 使用 **Key B**（`ADMIN_API_KEY`）认证，与 Key A 完全隔离。Worker 仅验证 Key B，不关心调用者身份。Admin 身份认证（Google OAuth + `ADMIN_EMAILS` 白名单）在 Next.js 服务端完成。所有通过白名单的 Admin 权限相等，不分级。
...
> **关键约束**：Key A 发往 `/api/admin/*` 会被拒绝，Key B 发往 `/api/v1/*` 也会被拒绝。两组 Key 严格绑定各自的路由前缀。

### 论坛用户 JWT 认证（Key A 体系）

```typescript
// middleware/auth.ts — 用于 /api/v1/* 写操作 + /api/v1/moderation/*
...
请求到达 Worker
  │
  ├─ /api/live ──────────────────────────→ 健康检查（无需任何认证）
  │
  ├─ /api/v1/* ──────── Key A 验证 ──────→
  │   ├─ GET /api/v1/*（公开读取）────────→ 通过（只需 Key A）
  │   ├─ /api/v1/auth/*（登录/刷新/登出）─→ 通过（只需 Key A）
  │   ├─ POST /api/v1/threads, posts ───→ authMiddleware → 论坛 JWT 必须
...
> **ADMIN_EMAILS 和 AUTH_GOOGLE_ID 不在 Worker 环境中**。这两个变量属于 Next.js 服务端环境（Admin Console），用于 Google OAuth 身份验证和白名单检查。Worker 不需要感知 Admin 身份。
...
| 管理员 Google OAuth 登录 → 封禁用户 | Playwright |
| 版主 → 论坛前端置顶主题 | Playwright |
| 速率限制触发 | Playwright |
| Key A 访问 admin 路由被拒 | Playwright |
| Key B 访问 v1 路由被拒 | Playwright |
...
> **ADMIN_EMAILS 和 AUTH_GOOGLE_ID**：这两个变量属于 Next.js 服务端环境（`apps/web/.env`），不在 Worker secrets 中配置。完整的 Next.js Admin 环境变量列表（含 `AUTH_GOOGLE_SECRET`、`AUTH_SECRET`、`AUTH_URL`）见 [04c §2.6](./04c-admin-console.md)。
...
```
论坛用户（Web + CLI）:
  Browser/CLI → Key A → Worker /api/v1/* → D1 用户表（论坛 JWT + KV refresh token）
  Auth source: Worker（D1 用户表）

Admin（Admin Console）:
  Browser → HttpOnly cookie → Next.js Server → 验证 Google OAuth session + ADMIN_EMAILS 白名单
