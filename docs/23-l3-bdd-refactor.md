...
| Project | 当前 testMatch | BDD testMatch |
|---------|---------------|---------------|
| `stateless` | 16 个文件名正则 | `bdd/auth.spec.ts`, `bdd/navigation.spec.ts`, `bdd/search.spec.ts`, `bdd/system.spec.ts`, `bdd/public.spec.ts` |
| `stateful` | 5 个文件名正则 | `bdd/content.spec.ts`, `bdd/social.spec.ts` |
| `mobile` | `mobile-layout.spec.ts` | `bdd/mobile.spec.ts` |
| `admin` | `admin/*.spec.ts` | `bdd/admin/*.spec.ts` |

Stage 1 迁移期需同时匹配旧/新路径，Stage 2 收敛到纯 BDD 路径。

### 4.2 stateful 保护

stateful project 中的 CRUD 测试（thread-crud、post-crud）依赖顺序执行。BDD 合并到 `content.spec.ts` 后，CRUD 测试必须在 `test.describe.serial` 块内。
...
| 批次 | 内容 | 工作量 |
|------|------|--------|
| 0.1 | 创建 `tests/e2e/bdd/fixtures.ts` + Stage 1 config commit | 极小 |
...
- [ ] `bun run test:e2e:browser` + `bun run test:e2e:admin` 全量通过
