...
const sharedState = vi.hoisted(() => ({
	capturedSubscribe: null as ((cb: () => void) => () => void) | null,
	capturedGetSnapshot: null as (() => "light" | "dark" | "system") | null,
	capturedGetServerSnapshot: null as (() => "light" | "dark" | "system") | null,
...
vi.mock("react", () => ({
	useSyncExternalStore(
		...
		getSnapshot: () => "light" | "dark" | "system",
		getServerSnapshot: () => "light" | "dark" | "system",
	) {
		sharedState.capturedSubscribe = subscribe;
		sharedState.capturedGetSnapshot = getSnapshot;
		sharedState.capturedGetServerSnapshot = getServerSnapshot;
		return getSnapshot();
	},
...
describe("useTheme — snapshot functions", () => {
	...
	it("getServerSnapshot always returns 'system'", () => {
		expect(sharedState.capturedGetServerSnapshot).not.toBeNull();
		expect(sharedState.capturedGetServerSnapshot?.()).toBe("system");
	});
	...
	it("getSnapshot returns 'system' when window is undefined (SSR guard)", () => {
		...
		expect(sharedState.capturedGetSnapshot?.()).toBe("system");
...
describe("useTheme — external store subscribe", () => {
	it("subscribe returns unsubscribe function", () => {
		useTheme();
		const unsub = sharedState.capturedSubscribe?.(() => {});
	...
	it("subscribe adds listener and unsubscribe removes it", () => {
		...
		const unsub = sharedState.capturedSubscribe?.(listener);
	...
	it("multiple subscribers all get notified", () => {
		...
		sharedState.capturedSubscribe?.(listener1);
		sharedState.capturedSubscribe?.(listener2);
