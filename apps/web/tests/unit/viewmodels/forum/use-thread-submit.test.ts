...
vi.mock("@/lib/api-client", () => ({
	apiClient: { post: vi.fn(async () => ({ data: { id: 999 } })) },
...
import {
	...
	validateContent,
	validateSubject,
} from "@/viewmodels/forum/use-thread-submit";
...
describe("submitThread", () => {
	it("calls apiClient.post with forumId, trimmed subject, and content", async () => {
		const id = await submitThread(5, "  Hello World  ", "<p>Body content here</p>");
