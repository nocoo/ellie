...
describe("proxy", () => {
	...
	it.each([true, false])(
		...
		async (prefetch) => {
			mockAuth.mockResolvedValue(null);
			const request = makeMockNextRequest("/threads/42");
			request.headers = new Headers({
				"x-ellie-prefetch": prefetch ? "0" : "1",
				...(prefetch ? { "next-router-prefetch": "1" } : {}),
			});
			await proxy(request);
			const options = vi.mocked(NextResponse.next).mock.calls.at(-1)?.[0];
			expect(new Headers(options?.request?.headers).get("x-ellie-prefetch")).toBe(
				prefetch ? "1" : "0",
			);
		},
	);
...
// ---------------------------------------------------------------------------
// proxy() — analytics ingest retired (doc/29)
// ---------------------------------------------------------------------------

describe("proxy() — analytics ingest retired (doc/29)", () => {
	const originalFetch = globalThis.fetch;
	const originalEnv = { ...process.env };
	...
	afterEach(() => {
		...
		process.env.WORKER_API_URL = originalEnv.WORKER_API_URL;
		process.env.FORUM_API_KEY = originalEnv.FORUM_API_KEY;
		if (originalEnv.ANALYTICS_INGEST_KEY === undefined) {
			Reflect.deleteProperty(process.env, "ANALYTICS_INGEST_KEY");
		} else {
			process.env.ANALYTICS_INGEST_KEY = originalEnv.ANALYTICS_INGEST_KEY;
		}
	});

	it("never posts to the retired ingest endpoint", async () => {
		process.env.WORKER_API_URL = "https://worker.example.com";
		process.env.FORUM_API_KEY = "test-key";
		...
		await proxy(makeMockNextRequest("/"));
		expect(calls.some((url) => url.includes("/api/internal/analytics/ingest"))).toBe(false);
	});
});
