...
it("does not fold miss+load or logical ops into overlapping chart totals", () => {
	expect(
		bucketCacheOpPoints([
			{ family: "forum:tree:v2", tsMinute: 1, op: "miss", count: 1 },
			{ family: "forum:tree:v2", tsMinute: 1, op: "load", count: 1 },
		]),
	).toEqual([{ tsMinute: 1, miss: 1, load: 1 }]);
	expect(
		bucketCacheOpPoints([
			{ family: "forum:tree:v2", tsMinute: 1, op: "read", count: 1 },
			{ family: "forum:tree:v2", tsMinute: 1, op: "write", count: 1 },
			{ family: "forum:tree:v2", tsMinute: 1, op: "kv-get", count: 1 },
			{ family: "forum:tree:v2", tsMinute: 1, op: "kv-put", count: 1 },
		]),
	).toEqual([{ tsMinute: 1, kvOps: 2 }]);
	expect(
		bucketCacheOpPoints([
			{ family: "forum:tree:v2", tsMinute: 1, op: "error", count: 1 },
			{ family: "forum:tree:v2", tsMinute: 1, op: "load-error", count: 1 },
		]),
	).toEqual([{ tsMinute: 1, error: 1 }]);
});

it("keeps miss, load, and physical KV calls on separate series and leaves skipped minutes as gaps", () => {
	render(
		<KvMetricsChart
			series={[
				{ family: "forum:tree:v2", tsMinute: 1, op: "hit", count: 3 },
				{ family: "forum:tree:v2", tsMinute: 1, op: "load", count: 1 },
				{ family: "forum:tree:v2", tsMinute: 1, op: "error", count: 2 },
	...
	expect(JSON.parse(charts[0].textContent ?? "[]")).toEqual([
		{ tsMinute: 1, hit: 3, load: 1, error: 2, kvOps: 4 },
		{ tsMinute: 2 },
		{ tsMinute: 3, miss: 5 },
	]);
