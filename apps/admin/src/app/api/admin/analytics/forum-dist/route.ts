// BFF: /api/admin/analytics/forum-dist?range=<>
// Forwards to worker GET /api/admin/analytics/forum-dist.
...
export const GET = createProxyHandler(async (request, admin) => {
	...
	const target = `/api/admin/analytics/forum-dist${search ? `?${search}` : ""}`;
